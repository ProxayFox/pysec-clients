...
6. Update only public documentation that no longer matches the implementation. Do not manually edit generated schemas or models; when regeneration is required, use `just schema-build` or `just schema-refresh`.
7. Validate incrementally: run the narrowest relevant test or docs command immediately after each substantive edit. For generated-contract changes, run `just schema-build-dry` and `just quality-schema`; run focused `just test tests/mde_client/... --skip-integration`; run `just docs-validate` when docs change; then run `just quality`. Do not run `just quality-full` unless Azure-backed integration coverage is intended and credentials are available.
