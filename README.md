...
| Generate MDE contracts | `just contracts-generate` |
| Check generated drift | `just contracts-check` |
| Diagnose contract state | `just contracts-doctor` |
| Contract quality gate | `just quality-contracts` |
...
pysec-clients/
    ...
    scripts/                   # authenticated metadata acquisition and compatibility shims
    tools/
        mde-contract-gen/      # standalone EDMX -> canonical contract generator
    src/
    ...
    tests/
        mde_contract_gen/      # generator, governance, and projection tests
...
## Canonical MDE Contracts

MDE response shapes are generated by the standalone
[`mde-contract-gen`](tools/mde-contract-gen/) workspace package. The
`mde-client` wheel vendors canonical `contract.json`, PyArrow `schema.py`, and
Draft 2020-12 `schema.schema.json` artifacts under
`mde_client/contracts/`; it has no XML-parser or code-generator runtime
dependency.

The authority chain is upstream XML plus checked-in overrides -> canonical
contract -> generated projections. See
[Canonical MDE source contracts](docs/mde_client/explanation/canonical-contracts.md)
for override, versioning, downstream registry, rollout, and rollback guidance.
...
4. The root `[tool.uv.workspace] members = ["src/*", "tools/*"]` picks up the new member
